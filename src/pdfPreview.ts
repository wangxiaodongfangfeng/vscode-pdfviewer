...
export class PdfPreview extends Disposable {
  ...
  private lookUpWord(word: string) {
    console.log("lookUpWord for " + word);
    const config = vscode.workspace.getConfiguration('pdf-preview');
    const lookupUrl = config.get('default.lookupUrl') as string;
    axios
      .post(lookupUrl, { word }, { headers: { 'Content-Type': 'application/json' } })
      .then((response) => {
  ...
  private getWebviewContents(): string {
    ...
    const settings = {
      ...
      defaults: {
        ...
        lookupUrl: config.get('default.lookupUrl') as string
      },
    };
...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src ${cspSource} ${settings.defaults.lookupUrl}; script-src 'unsafe-inline' ${cspSource}; style-src 'unsafe-inline' ${cspSource}; img-src blob: data: ${cspSource};">
